import type { Competitor } from "@/content/compare";
import { Cite } from "./Cite";

/** Server-rendered FAQ; same visual language as the home page FAQ, no client JS. */
export function CompareFaq({ c }: { c: Competitor }) {
  return (
      ...
      {c.faq.map((item, i) => (
        <li key={item.q} data-reveal style={{ ["--reveal-i" as string]: i % 3 }} className="border-b border-line">
          <details className="group">
            ...
            <p className="max-w-2xl pb-6 pr-12 text-[0.98rem] leading-relaxed text-fg-2 [animation:fade-rise_0.4s_var(--ease-out)_both]">
              {item.a}
